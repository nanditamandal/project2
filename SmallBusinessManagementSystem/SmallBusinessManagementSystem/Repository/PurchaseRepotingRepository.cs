...
namespace SmallBusinessManagementSystem.Repository
{
    class PurchaseRepotingRepository
    {
        ...
        public List<PurchaseRepotingModel> SearchValueByName(PurchaseRepotingModel purchaseRepotingModel)
        {
            string connectionString = @"Server=DESKTOP-077IM0O;DataBase=Project1DB;Integrated Security=true";
            SqlConnection sqlConnection = new SqlConnection(connectionString);

            string CommandString = @"SELECT * FROM PurchaseReport WHERE ProductName='" + purchaseRepotingModel.Name + "' ";
